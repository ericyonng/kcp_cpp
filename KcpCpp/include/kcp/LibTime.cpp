...
#include <kcp/TimeSlice.h>
#include <kcp/util/StringUtil.h>
#include <kcp/util/TimeUtil.h>

static inline const KCP_CPP_NS::LibString &GetZeroTimeString()
{
    static const KCP_CPP_NS::LibString __g_ZeroTimeString = "1970-01-01 00:00:00.000000";
...
LibString LibTime::ToString() const
{
    ...
    LibString repr;   
    return repr.AppendFormat("%s.%06d", Format().c_str(), localTime % TimeDefs::MICRO_SECOND_PER_SECOND);
}
...
LibString LibTime::ToStringOfMillSecondPrecision() const
{
    ...
    auto localTime = _rawTime - static_cast<Int64>(TimeUtil::GetTimeZone()*TimeDefs::MICRO_SECOND_PER_SECOND);
    LibString repr;   
    return repr.AppendFormat("%s.%03d", Format().c_str(), localTime%TimeDefs::MICRO_SECOND_PER_SECOND / TimeDefs::MICRO_SECOND_PER_MILLI_SECOND);
}
